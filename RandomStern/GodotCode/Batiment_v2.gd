...
func _init():
	randomize()
	self.couleur = randi()%4

func _ready():
	get_node("Sprite").set_frame(self.couleur)
...
func dessine():
	if(!deja_dessine):
		...
		if(self.type=="saloon"):
			...
			self.interieur.ajout(PIANO.instance())
		elif(self.type=="entrepot"):
			self.interieur.largeur=TAILLE_CASE*4
			self.interieur.longueur=TAILLE_CASE*3
			...
			self.interieur.largeur=TAILLE_CASE*4
			self.interieur.longueur=TAILLE_CASE*6
			var nb_presentoirs = randi()%2+2
			...
			self.interieur.largeur=TAILLE_CASE*5
			self.interieur.longueur=TAILLE_CASE*5
